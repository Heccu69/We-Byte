...
- Начинает его утаскивать **вправо** (по оси X)
...
- Процесс продолжается, пока есть коржи и тараканы
...
CakeManager автоматически собирает коржи при старте и каждые 2 секунды:
- Ищет все объекты с компонентом **PickupObject** типа **Korzh**
- **Фильтрует коржи:**
  - ✗ Пропускает коржи на конвейере (связанные с платформой)
  - ✗ Пропускает движущиеся коржи (скорость > 0.1)
  - ✗ Пропускает коржи в руках игрока
- Проверяет, что они находятся рядом с тарелкой (в радиусе 9 единиц)
...
- ✅ `"Корж утащен! Осталось коржей: X"` - корж уничтожен и утащен вправо
...
3. **Корж утаскивается:**
   - Нижний корж должен медленно двигаться **вправо**
...
### Тараканы останавливаются перед тарелкой:
- Проверьте значение **stopDistance** в EnemyMove (должно быть маленьким, например 0.2)
- Убедитесь, что тараканы игнорируют коллизии с коржами (автоматически настраивается)
- Проверьте, что целевая позиция - это тарелка UnderCake, а не коржи
